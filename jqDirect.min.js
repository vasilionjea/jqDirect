...
ihtml+="</style>";ihtml+="</head>";ihtml+="<body>";ihtml+=self.$directions_output.html();ihtml+=self.$el.wrap("<div />").parent().html();ihtml+="</body></html>";self.$el.unwrap("<div />");document.body.appendChild(iframe);i_window=iframe.contentWindow;i_document=iframe.contentDocument||i_window.document;i_document.open();i_document.write(ihtml);i_document.close();$(i_document).find("#"+self.$el.prop("id")).css({width:"100%",height:"380px"});i_window.focus();window.setTimeout(function(){i_window.print()},
13)}});$.fn.jqDirect=function(options,args){return this.each(function(){var $this=$(this),instance=$this.data("gdirections");if(!instance){instance=new jqDirect(this,options);$this.data("gdirections",instance._init())}if(typeof options=="string"&&!/_init|_createMap|_geocode_address/.test(options))instance[options](args)})}})(jQuery);